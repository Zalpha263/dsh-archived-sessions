...
- **仍在内存中的会话走「待删除队列」**：归档只把会话从列表隐藏、**不会停止它**，所以刚归档的那个会话一定还在内存里；Windows 上宿主还占着日志文件句柄，直接删会失败。现在的做法是：先请 DSH 官方的 `archiveSession(id, { stopActivity: true })` 停止它的运行中工作（turn、子代理后代、它拥有的后台任务与计划），然后**立即尝试**删除；若日志仍被占用，就把它记进**待删除队列**，等它退出内存时自动完成（最迟下次重启 DSH —— 刚启动时没有会话在内存里）。队列状态显示在列表上方，界面上的标记是「仍在内存 · 删除时先停其活动」。
- **待删除队列的落盘位置**：`$DSH_HOME/archived-sessions-pending-delete.json`（插件自有的一个小 JSON 文件，原子写入）。这是本插件唯一会写的数据文件。
