...
`;
		...
		function ArchivedSessionsSection(props) {
			...
			const confirmDelete = (item) => {
				...
				setNotice(null);
				remote().deleteSession(item.sessionId, deleteInput, item.title)
					.then(() => {
		...
		async function apply(ctx) {
			...
			function remote() {
				return {
					...
					deleteSession: (sessionId, titleConfirm, displayedTitle) => call("deleteSession", sessionId, titleConfirm, displayedTitle)
				};
			}
