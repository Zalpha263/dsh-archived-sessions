...
.as-warn{color:var(--dsw-alias-state-warn-primary);margin:0;font-size:13px;line-height:20px}
				...
				if (item.live) {
					confirmPanel = React.createElement('div', { className: 'as-confirm' },
						React.createElement('p', { className: 'as-warn', role: 'alert' },
							'该会话当前已打开（位于内存中），无法删除。直接删除打开中的会话会导致其日志被重新写回、再次出现在工作区。请重启 DSH 后，再从本页删除该会话。'),
					...
					confirmPanel = React.createElement('div', { className: 'as-confirm' },
						...
						React.createElement('input', {
							...
							onKeyDown: (event) => {
								if (event.key === 'Enter' && deleteInput === item.title && !disabled) onConfirmDelete(item);
						...
						React.createElement('div', { className: 'as-confirmRow' },
							...
							React.createElement('button', {
								...
								disabled: deleteInput !== item.title || disabled,
				...
				React.createElement('div', { className: 'as-cardHead' },
					React.createElement('div', { className: 'as-cardTitles' },
						React.createElement('span', { className: 'as-titleRow' },
							...
							item.live && React.createElement('span', { className: 'as-tag as-tag-live' }, '无法删除 · 位于内存中'),
						),
