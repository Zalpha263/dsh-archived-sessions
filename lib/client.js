...
`;
		...
		const CONTRIBUTION = {
			...
			descriptors: [
				...
				{
					id: "dsh-archived-sessions#archivedSessions/scanOrphans",
					service: "archivedSessions",
					namespace: "archivedSessions",
					method: "scanOrphans",
					invocation: { kind: "direct" },
					parameters: [],
					result: strictCodec("dsh-archived-sessions#archivedSessions/scanOrphans:result"),
					sourceLocation: { "file": "dsh-archived-sessions/lib/client.js", "line": 1, "column": 1 }
				},
				{
					id: "dsh-archived-sessions#archivedSessions/purgeOrphans",
					service: "archivedSessions",
					namespace: "archivedSessions",
					method: "purgeOrphans",
					invocation: { kind: "direct" },
					parameters: [
						{ name: "sessionIds", wire: "sessionIds", source: "json", codec: strictCodec("dsh-archived-sessions#archivedSessions/purgeOrphans:sessionIds") },
						{ name: "confirmWord", wire: "confirmWord", source: "json", codec: strictCodec("dsh-archived-sessions#archivedSessions/purgeOrphans:confirmWord") }
					],
					result: strictCodec("dsh-archived-sessions#archivedSessions/purgeOrphans:result"),
					sourceLocation: { "file": "dsh-archived-sessions/lib/client.js", "line": 1, "column": 1 }
				}
			]
		};
		...
		function ArchivedSessionsSection(props) {
			...
			// ---- Orphan scan (read-only until the user confirms) ---------------
			// An orphan is a session whose log exists but that no workspace accounts
			// for and that is absent from the archive set: invisible on this page,
			// ungrouped in the sidebar. The Host reports whether each one is still
			// readable, so a normal ungrouped session is recognisable.
			const [orphans, setOrphans] = React.useState(null);
			const [orphanSelected, setOrphanSelected] = React.useState([]);
			const [orphanAck, setOrphanAck] = React.useState(false);
			const [orphanWord, setOrphanWord] = React.useState('');
			...
			const scanOrphans = () => {
				...
				remote().scanOrphans().then((result) => {
					if (!result || result.ok !== true) throw new Error((result && result.error) || '扫描失败');
					setOrphans(result);
					setOrphanSelected([]);
					setOrphanAck(false);
					setOrphanWord('');
				}).catch((error) => {
					setNotice({ kind: 'error', text: '孤儿会话扫描失败：' + String((error && error.message) || error) });
				}).then(() => setOrphanBusy(false));
			};
			const runPurgeOrphans = () => {
				if (orphans === null) return;
				setOrphanBusy(true);
				remote().purgeOrphans(orphanSelected.slice(), orphanWord).then((result) => {
					setOrphanResults(result);
					const summary = result && result.summary ? result.summary : null;
					setNotice({
						kind: summary !== null && summary.failed > 0 ? 'error' : 'success',
						text: summary === null ? '孤儿清理已完成。'
							: '孤儿清理：已清除 ' + summary.deleted + ' 个'
								+ (summary.pending > 0 ? '，待删除 ' + summary.pending + ' 个' : '')
					...
					setNotice({ kind: 'error', text: '孤儿清理失败：' + String((error && error.message) || error) });
				}).then(() => setOrphanBusy(false));
			};
			...
			return React.createElement('div', { className: 'as-section' },
				...
				// One action row for the whole section: 批量删除 (only meaningful with
				// archived sessions) turns into the selection toolbar in bulk mode, and
				// 扫描孤儿会话 stays available next to it — orphans are NOT archived, so
				// it must render even when the archive list is empty.
				phase === 'ready' && React.createElement('div', { className: 'as-bulkBar' },
					merged.length > 0 && !bulkMode && React.createElement('button', {
						type: 'button', className: 'as-btn', onClick: () => setBulkMode(true),
					}, '批量删除'),
					// The toolbar follows bulkMode alone: emptying the archive list from
					// inside bulk mode must never take 退出批量 away.
					...
					React.createElement('button', {
						...
						onClick: scanOrphans,
						disabled: orphanBusy,
						title: '找出「日志还在、但不属于任何工作区、也不在归档列表」的会话（本页看不到它们，侧边栏显示为未分类）',
					}, orphanBusy && orphans === null ? '扫描中…' : '扫描孤儿会话'),
				),
				orphans !== null && React.createElement('div', { className: 'as-confirm as-bulkConfirm' },
					React.createElement('p', { className: 'as-hint' },
						'孤儿 = 日志还在、但不属于任何工作区、也不在归档列表的会话：本页看不到它们，侧边栏会显示成未分类。'),
					React.createElement('p', { className: 'as-warn' },
						'扫描了 ' + orphans.scanned + ' 个会话，找到 ' + orphans.items.length + ' 个孤儿。' +
						'「可读」表示日志内容仍完整（多半是正常的未分组会话，删掉会丢内容）；' +
						'「不可读」表示数据已经缺失（本次删除回归的产物，清理它们是安全的）。'),
					orphans.items.length === 0 && React.createElement('p', { className: 'as-hint' }, '没有孤儿会话。'),
					orphans.items.length > 0 && React.createElement('ul', { className: 'as-bulkList' },
						orphans.items.map((entry) => React.createElement('li', { key: entry.sessionId, className: 'as-bulkItem' },
							React.createElement('input', {
								type: 'checkbox',
								className: 'as-check',
								checked: orphanSelected.includes(entry.sessionId),
								onChange: () => setOrphanSelected((current) => current.includes(entry.sessionId)
									? current.filter((id) => id !== entry.sessionId)
									: current.concat([entry.sessionId])),
							}),
							' ' + entry.sessionId,
							React.createElement('span', { className: 'as-hint' },
								' · ' + (entry.readable
									? '可读（' + (entry.eventCount === null ? '?' : entry.eventCount) + ' 个事件）'
									: '不可读 / 数据缺失')
								+ (entry.live ? ' · 仍在内存' : '')
								+ (entry.createdAt === null ? '' : ' · 创建于 ' + formatDateTime(entry.createdAt))
								+ (entry.cwd ? ' · ' + entry.cwd : '')),
						)),
					),
					orphans.items.length > 0 && React.createElement(React.Fragment, null,
						React.createElement('label', { className: 'as-bulkAck' },
							React.createElement('input', {
								type: 'checkbox', checked: orphanAck,
								onChange: (event) => setOrphanAck(event.target.checked),
							}),
							'我知道清理不可恢复',
						),
						React.createElement('input', {
							className: 'as-input', value: orphanWord,
							placeholder: '输入「' + confirmWord + '」以确认清理',
							onChange: (event) => setOrphanWord(event.target.value),
						}),
						React.createElement('div', { className: 'as-confirmRow' },
							React.createElement('button', {
								type: 'button', className: 'as-btn', onClick: () => setOrphans(null), disabled: orphanBusy,
							}, '关闭'),
							React.createElement('button', {
								type: 'button', className: 'as-btn as-btn-danger', onClick: runPurgeOrphans,
								disabled: orphanBusy || !orphanAck || orphanWord.trim() !== confirmWord || orphanSelected.length === 0,
							}, orphanBusy ? '清理中…' : '清理所选（' + orphanSelected.length + '）'),
						),
					),
				),
				orphanResults !== null && React.createElement('div', { className: 'as-bulkResults' },
					React.createElement('p', { className: 'as-hint' }, '孤儿清理结果：'),
					React.createElement('ul', { className: 'as-bulkList' },
						orphanResults.results.map((row) => React.createElement('li', { key: row.sessionId, className: 'as-bulkItem' },
							(row.status === 'deleted' ? '✅ 已清除 '
								: row.status === 'pending' ? '⏳ 待删除 '
									: row.status === 'failed' ? '❌ 失败 ' : '⏭ 跳过 ') + row.sessionId,
							row.reason ? React.createElement('span', { className: 'as-hint' }, ' —— ' + row.reason) : null,
						)),
					),
					React.createElement('button', {
						type: 'button', className: 'as-btn', onClick: () => setOrphanResults(null),
					}, '收起结果'),
				),
		...
		async function apply(ctx) {
			...
			function remote() {
				return {
					...
					scanOrphans: () => call("scanOrphans"),
					purgeOrphans: (sessionIds, confirmWord) => call("purgeOrphans", sessionIds, confirmWord)
				};
			}
