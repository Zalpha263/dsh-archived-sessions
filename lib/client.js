...
`;
		...
		function SessionCard(props) {
			const { item, expanded, preview, confirming, busy, disabled,
				onTogglePreview, onRestore, onStartDelete, onCancelDelete, onConfirmDelete, onForceDelete,
				...
				if (item.live) {
					...
					confirmPanel = React.createElement('div', { className: 'as-confirm' },
						...
						childrenAck !== null && childrenAck !== undefined && React.createElement('p', { className: 'as-warn', role: 'alert' },
							'该会话有 ' + childrenAck.count + ' 个子会话。删除父会话不会删除它们——子会话的日志是独立的，删除后会保留，只是不再有父会话关联。确认无误请点「仍要删除父会话」。'),
						React.createElement('div', { className: 'as-confirmRow' },
							...
							(childrenAck !== null && childrenAck !== undefined)
								...
								}, busyOn && busy.kind === 'delete' ? '删除中…' : '仍要删除父会话')
								: React.createElement('button', {
									type: 'button',
									className: 'as-btn as-btn-danger',
		...
		function ArchivedSessionsSection(props) {
			...
			/* Shared delete runner. A first attempt on a session that still has
			   ...
			   the UI can warn and the user can confirm a parent-only delete. */
			const runDelete = (item, allowChildren) => {
				setBusy({ id: item.sessionId, kind: 'delete' });
				setNotice(null);
				remote().deleteSession(item.sessionId, deleteInput, item.title, allowChildren)
					.then((result) => {
						if (result && result.needsChildrenAck === true) {
							const count = typeof result.childCount === 'number' ? result.childCount : 0;
							setChildrenAck({ id: item.sessionId, count });
							setNotice({ kind: 'error', text: '该会话有 ' + count + ' 个子会话。删除父会话不会删除它们；确认无误请点「仍要删除父会话」。' });
							return;
						}
			...
			const confirmDelete = (item) => runDelete(item, false);
			const forceDelete = (item) => runDelete(item, true);
		...
		async function apply(ctx) {
			...
			function remote() {
				return {
					...
					deleteSession: (sessionId, titleConfirm, displayedTitle, allowChildren) => call("deleteSession", sessionId, titleConfirm, displayedTitle, allowChildren)
				};
			}
