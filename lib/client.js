...
`;
		...
		function SessionCard(props) {
			const { item, expanded, preview, confirming, busy, disabled,
				...
				deleteInput, deleteHint, onDeleteInputChange } = props;
				...
				if (item.live) {
					...
					confirmPanel = React.createElement('div', { className: 'as-confirm' },
						...
						React.createElement('input', {
							...
							onKeyDown: (event) => {
								if (event.key === 'Enter' && deleteInput.trim().length > 0 && !disabled) onConfirmDelete(item);
		...
		function ArchivedSessionsSection(props) {
			...
			const startDelete = (item) => {
				...
				setDeleteInput('');
				/* v1.2.1: tell the user what to type up front — the Host
				   confirms against durable title → cwd basename → session id,
				   which can differ from the displayed summary title when the
