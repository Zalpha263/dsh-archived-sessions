...
//   - On-disk layout (dsh-session-persistence-jsonl 0.1.2-rc.1):
//     <root>/<projectKey(cwd)>/<encodeSegment(id)>/session.jsonl(.zstd).
//     encodeSegment() is the identity for every DSH-minted id (all ids are
//     [A-Za-z0-9._-]) but is NOT publicly exported (the package exports only
//     JsonlSessionPersistence), so the delete path validates the literal id;
//     see the path check in deleteSession.
...
// session-query corpus with a constant revision placeholder. An empty
...
class ArchivedSessionsService extends TypertRemoteService {
  ...
  async deleteSession(sessionId, titleConfirm) {
    ...
    if (titleConfirm.trim() !== expected) {
      /* v1.2.1: the client's displayed title may use a different fallback
         (displayTitle vs our durable→cwd→id chain) when the session summary
         is missing. Return the expected value so the UI can show exactly
         what to type instead of a dead-end mismatch. */
      throw new Error('输入的标题与「' + expected + '」不一致，已取消删除。请输入：' + expected)
    }
    ...
    if (snap) {
      ...
      // locate() derives <root>/<projectKey(cwd)>/<encodeSegment(id)>/
      // session.jsonl(.zstd) straight from the header, so these checks are a
      // defensive re-verification of the derived shape rather than the sole
      // guard. `dirName === sessionId` holds because encodeSegment is the
      // identity for every DSH-minted id (uuid / session-<uuid> / <id>-session-<uuid>;
      // all [A-Za-z0-9._-]); an id containing `~` or another unsafe unit would
      // appear as ~XXXX here and be REFUSED by design — encodeSegment is an
      // internal symbol of dsh-session-persistence-jsonl (not publicly
      // exported), so the check cannot be widened to the encoded form.
      ...
      // v1.2.1: platform-appropriate removal command — the previous hardcoded
      // PowerShell Remove-Item made deletion impossible on macOS/Linux.
      ...
      // An unverifiable listing aborts the same way a surviving log does:
      // the shell run reported success but neither listing source confirmed
      // it, and proceeding on a phantom log would resurface the session as
      // an ungrouped phantom. A retry is safe — it re-validates before
      // deleting again, and once the log is gone the retry skips this step.
      const after = await loadSnapshots(ctx)
      if (!after.ok) {
        throw new Error('无法确认日志文件是否已删除，已中止删除')
      }
      if (after.snapshots.some((entry) => entry.header.id === sessionId)) {
