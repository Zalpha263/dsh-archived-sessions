...
async function readPendingIds() {
  try {
    ...
    const ids = parsed && Array.isArray(parsed.sessionIds) ? parsed.sessionIds : []
    return ids.filter((id) => typeof id === 'string' && id !== '')
  } catch {
    return [] // absent or unreadable: an empty queue is the safe default
  }
}

async function writePendingIds(ids) {
  const file = pendingFilePath()
  const tmp = file + '.tmp'
  await mkdir(dirname(file), { recursive: true })
  await writeFile(tmp, JSON.stringify({ sessionIds: ids, updatedAt: Date.now() }, null, 2), 'utf8')
  await rename(tmp, file) // atomic replace: a crash never truncates the queue
}

async function enqueuePending(ids) {
  const wanted = (Array.isArray(ids) ? ids : [ids]).filter((id) => typeof id === 'string' && id !== '')
  if (wanted.length === 0) return []
  const current = await readPendingIds()
  const merged = current.slice()
  for (const id of wanted) if (!merged.includes(id)) merged.push(id)
  if (merged.length !== current.length) await writePendingIds(merged)
  return merged
}

async function dequeuePending(ids) {
  const done = new Set((Array.isArray(ids) ? ids : [ids]).filter((id) => typeof id === 'string'))
  if (done.size === 0) return []
  const current = await readPendingIds()
  const kept = current.filter((id) => !done.has(id))
  if (kept.length !== current.length) await writePendingIds(kept)
  return kept
}
...
class ArchivedSessionsService extends TypertRemoteService {
  ...
  async deleteSession(sessionId, titleConfirm, displayedTitle, allowChildren, cascadeChildren) {
    ...
    // Mutation phase. A cold session keeps the original contract (every failure
    // throws before durable records are touched). A LIVE session instead falls
    // back to the pending queue: the stops requested above are not awaited by the
    // seam, so the very first attempt may well fail — the queue retries it.
    try {
      ...
      await purgeRecords(ctx, [sessionId].concat(deletedChildren))
      await dequeuePending([sessionId].concat(deletedChildren))
  ...
  async deleteSessions(sessionIds, cascadeMode, confirmWord) {
    ...
    for (const sessionId of sessionIds) {
      ...
      try {
        ...
        const reason = String((error && error.message) || error)
        if (!wasLive) {
          results.push({ sessionId, status: 'failed', reason, deletedChildren: [], skippedChildren: split.live })
          continue
        }
        await enqueuePending([sessionId])
        results.push({ sessionId, status: 'pending', reason, stopError, deletedChildren: [], skippedChildren: split.live })
      }
    }
